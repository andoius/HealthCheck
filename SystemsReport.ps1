...
foreach ($computer in $computers) {
    ...
    $OSServicePack = $OperatingSystem.CSDVersion
    ...
    $ComputerInfo = Get-WmiObject -Class win32_computersystem -computername $computer | Select-Object Domain, Manufacturer, Model, Name
    ...
    $ServerMake = $ComputerInfo.Manufacturer
    $ServerModel = $ComputerInfo.Model
    $ServerDomain = $ComputerInfo.Domain
    $ServerName = $computer
    $ServerProcessor = $Processor.Name
    $ServerCores = $Processor.NumberOfCores
    $ServerLogicalProcessors = $Processor.NumberOfLogicalProcessors
    $ServerProcessorSpeed = $Processor.CurrentClockSpeed
	...
	<div id="report">
	<p><h2>$computer Configuration Report</p></h2>
	<h3>System Info</h3>
	<table class="list">
	<tr>
	<td>System Uptime</td>
    ...
    <h3>Hardware Info</h3>
	<p>Details of Server Hardware Configuration.</p>
	...
	<td>$ServerLogicalProcessors</td>
	</tr>
    <tr>
	<td>Speed</td>
	<td>$ServerProcessorSpeed MHz</td>
	</tr>
    <tr>
	</table>
...
Invoke-Expression ((Get-Location).Path + "\report.html")